...
import { Eye, EyeOff, ArrowRight, ShieldCheck, ArrowLeft, Check, X, Wrench } from 'lucide-react';
import { ImageWithFallback } from '../figma/ImageWithFallback';
import HouziiLogo from '../../../imports/Group1410124151';

interface ProCreateAccountProps {
  onContinue: (data: { fullName: string; email: string; phone: string }) => void;
  onLogin: () => void;
}

export const ProCreateAccount: React.FC<ProCreateAccountProps> = ({ onContinue, onLogin }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-white">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden rounded-r-[32px]">
        <ImageWithFallback
          src="https://images.unsplash.com/photo-1770822662903-aa69f6aa9b03?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxwcm9mZXNzaW9uYWwlMjBzdXJ2ZXlvciUyMGVuZ2luZWVyJTIwd29ya2luZyUyMGNvbnN0cnVjdGlvbnxlbnwxfHx8fDE3NzM3MDUzOTB8MA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral"
        ...
        <Link to="/professional" className="absolute top-8 right-8 flex items-center gap-2 text-white/70 hover:text-white transition-colors bg-white/10 hover:bg-white/20 backdrop-blur-sm px-4 py-2 rounded-full">
          <ArrowLeft className="w-4 h-4" />
          <span className="text-sm font-bold">Back to Professional page</span>
        </Link>
        <div className="absolute bottom-16 left-8 right-8">
          ...
          >
            Join Nigeria's first trust-verified professional marketplace. Get matched with clients, complete jobs, and get paid securely.
          </motion.p>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-16">
        ...
        >
          ...
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-primary/5 border border-primary/15 rounded-full mb-4">
            ...
            <span className="text-primary text-xs font-black uppercase tracking-widest">Professional Account</span>
          </div>

          <h1 className="text-slate-900 font-black text-3xl mb-2">Create Your Professional Profile</h1>
          <p className="text-slate-400 font-medium mb-10">Get verified, receive job requests, and earn with Houzii.</p>
