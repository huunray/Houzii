...
import { Eye, EyeOff, ArrowRight, ShieldCheck, Check, X, Home, ArrowLeft } from 'lucide-react';
import { ImageWithFallback } from '../figma/ImageWithFallback';
import HouziiLogo from '../../../imports/Group1410124151';

interface OwnerCreateAccountProps {
  onContinue: (data: { fullName: string; email: string; phone: string }) => void;
  onLogin: () => void;
}

export const OwnerCreateAccount: React.FC<OwnerCreateAccountProps> = ({ onContinue, onLogin }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-white">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden rounded-r-[32px]">
        <ImageWithFallback
          src="https://images.unsplash.com/photo-1742490411180-42d74efa4b1a?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxsdXh1cnklMjBhZnJpY2FuJTIwcHJvcGVydHklMjBvd25lciUyMG1vZGVybiUyMGhvbWV8ZW58MXx8fHwxNzczNzA0MTcwfDA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral"
        ...
        <Link to="/owners" className="absolute top-8 right-8 flex items-center gap-2 text-white/70 hover:text-white transition-colors bg-white/10 hover:bg-white/20 backdrop-blur-sm px-4 py-2 rounded-full">
          <ArrowLeft className="w-4 h-4" />
          <span className="text-sm font-bold">Back to Owners page</span>
        </Link>
        <div className="absolute bottom-12 left-8 right-8">
          <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-6 -translate-y-[44px]">
            <p className="text-white/80 text-sm font-bold mb-2">Why list with Houzii?</p>
            <div className="space-y-2.5">
              {[
                'Reach 50,000+ verified buyers & tenants',
                'Automated rent collection & escrow',
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-16">
        ...
        >
          ...
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-primary/5 border border-primary/15 rounded-full mb-4">
            ...
            <span className="text-primary text-xs font-black uppercase tracking-widest">Owner Account</span>
          </div>

          <h1 className="text-slate-900 font-black text-3xl mb-2">Create Your Owner Profile</h1>
          <p className="text-slate-400 font-medium mb-10">List, manage, and grow your property portfolio with Houzii.</p>
          ...
          <div className="mt-6 flex gap-3">
            <button className="flex-1 py-4 bg-white border border-slate-200 hover:border-slate-300 text-slate-700 rounded-2xl font-bold transition-all flex items-center justify-center gap-3 hover:shadow-sm">
